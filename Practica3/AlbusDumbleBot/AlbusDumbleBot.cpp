...
size_t hash_game_state::operator() (const GameState& state) const
{
  string s;
        ...
        bound alpha, bound beta, transposition_table& table)
{
    ...
    if (debug) {
        cerr << "[AlphaBeta]: Entra algoritmo,  con profundidad " << depth <<
             " y maximize " << root.is_max_node() <<  endl;
    }
    ...
    if (value_in_hash != table.end()) {
        ...
        if (debug) {
            cerr << "Nodo encontrado en tabla " << bound_in_hash.lower << " " <<
                 bound_in_hash.upper << endl;
        }
    ...
    if ( depth == 0 || root.is_terminal() ) {
        ...
        if (debug) {
            cerr << "[AlphaBeta]: Nodo Hoja, devuelve valor: " << ret._bound << " " <<
                 ret._action << endl;
        }
    ...
    if (debug) {
        cerr << "[AlphaBeta]: Devuelve Valor: " << ret._bound << " " << ret._action <<
             endl;
    }
                              ...
                              transposition_table& table)
{
    ...
    do {
        ...
        if (debug) {
            cerr << "[MTDF]: Aproximación actual " << ret._bound << " " << ret._action <<
                 endl;
        }
    ...
    if (debug) {
        cerr << "[MTDF]: Bound and Action: " << ret._bound << " " << ret._action <<
             endl;
    }
...
Move AlbusDumbleBot::nextMove(const vector<Move>& adversary, const GameState& state)
{
